...
2026-07-31 — 🎯 Player props shipped: over/under on player stat lines mixed into regular slates (classic/confidence), attached to a game for lock time + grading. Lines: manual, or blank = auto-suggested from Sleeper's free weekly projections (NFL). Grading: automatic from ESPN boxscores when the game goes final (verified against a real playoff game end-to-end through the production cron path: Allen 273 vs 249.5 → Over, game + props in one tick, 2 upstream fetches), manual Over/Under/Push for hand-entered games. Props ride the TV ticker. One migration.
